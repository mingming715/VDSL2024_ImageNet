...
from efficientnet_pytorch import EfficientNet_ # EfficientNet_ for No BlurPool
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # LTN optimizer
    optimizer = torch.optim.SGD(P.parameters(), args.lr,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, P, criterion, optimizer, epoch, args)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if epoch == args.start_epoch:
                ...
                checkpoint = f'checkpoint_{args.arch[-2:]}_{timestamp}.pth.tar'
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for idx in range(1000):
        one_hot_vector = np.zeros(1000)
        one_hot_vector[idx] = 1
        one_hot_vector_tensor = torch.tensor(one_hot_vector, dtype=torch.float32)
        globals()[f"l_{idx}"] = ltn.Constant(one_hot_vector_tensor)
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        for class_idx in range(1000):
            if class_idx in target:
                print(class_idx)
                class_data = images[target == class_idx]
                x_class = ltn.Variable(f"x_{class_idx}", class_data)
                l_class = globals()[f"l_{class_idx}"]
                clauses.append(Forall(x_class, model(x_class, l_class, training=True)))
            #print("x_class:", x_class)


        sat_agg = SatAgg(*clauses)
        loss = 1.0 - sat_agg
        print(loss)
        ...
        acc1, acc5 = accuracy(model.logits_model(images), target, topk=(1, 5))
...
def save_checkpoint(state, is_best, args, filename='checkpoint.pth.tar', timestamp=0):
    ...
    if is_best:
        shutil.copyfile(filename, f'model_best_{args.arch[-2:]}_{timestamp}.pth.tar')
